...
public class EventHandlerProcessor extends AbstractProcessor
{
    ...
    private void processRound(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv)
    {
        ...
        for (Element element : annotatedElements)
        {
            ...
            String remappedTargetMethod = treeRemapper.mapMethodName(annotation.targetClass(), annotation.targetMethodName(), annotation.targetMethodSignature());
            String remappedTargetMethodSignature = treeRemapper.mapMethodDesc(annotation.targetMethodSignature());
            ...
            MethodModifier methodModifier = new MethodModifier(
                    annotation.type(), remappedTargetMethod, remappedTargetMethodSignature,
                    eventHandlerClassName, eventHandlerMethodName,
                    executableElementDescriptor, annotation.targetMethodIsStatic());

            classModifier.addModifier(methodModifier);
        }
    }
    ...
    private String getExpectedDescriptor(EventHandler annotation) throws Exception
    {
        ...
        {
            MethodType targetMethodType = MethodType.fromMethodDescriptorString(annotation.targetMethodSignature(), EventHandlerProcessor.class.getClassLoader());
